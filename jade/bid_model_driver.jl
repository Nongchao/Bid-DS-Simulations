...
load_model = "sixty_percent_fixed_load" # ARGS[2]  # "fixed_load", "sixty_percent_fixed_load", "bid_in_load"
...
start_date = Dates.DateTime("2020-08-26T00:00:00")
thermal_formulation = "basic" # "basic", "standard"

proj = Pkg.project().path
proj_path = dirname(proj)

result_file_path = joinpath(proj_path, "results", "tolerance_1e_neg_4_new_start_date_bid_model"*"_$congestion"*"_$load_model"*"_carbon_price_"*"$carbon_pricing"*"_steps_"*"$days"*"_thermal_formulation_"*"$thermal_formulation"*"_data_center_"*"$number_of_data_center")
...
emission_cost[1, :SO2] = 0.0
emission_cost[1, :NOX] = 0.0
emission_cost[1, :Part] = 0.0
...
for l in data_center # get_components(PowerLoad, sys)
    ...
    # load_ts = values(get_time_series_array(SingleTimeSeries, l, "max_active_power"))
    load_ts = get_time_series_values(SingleTimeSeries, l, "max_active_power")
    if load_model == "fixed_load"
        bid_ts = collect([(-332*load_ts[t], load_ts[t])] for t = 1:length(load_ts))
    elseif load_model == "sixty_percent_fixed_load"
        bid_ts = collect([(-533.333333*load_ts[t]*0.6, load_ts[t]*0.6)] for t = 1:length(load_ts))
    elseif load_model == "bid_in_load"
        ...
        bid_ts = collect([(0.0, 0.0), (-1000.0*load_ts[t]/5, load_ts[t]/5), (-(1000+500.0)*load_ts[t]/5, load_ts[t]/5*2), (-(1000.0+500.0+100.0)*load_ts[t]/5, load_ts[t]/5*3), (-(1000.0+500.0+100.0+50.0)*load_ts[t]/5, load_ts[t]/5*4), (-(1000.0+500.0+100.0+50.0+10.0)*load_ts[t]/5, load_ts[t])] for t = 1:length(load_ts))
    else
        error("Unexpected value: $load_model")
    end
    bid_data = TimeArray(timestamp, bid_ts)
    _bid_time_series = SingleTimeSeries("variable_cost", bid_data)
...
if load_model == "fixed_load"
    set_device_model!(template, DeviceModel(FlexibleLoad, FixDispatchablePowerLoad))
elseif load_model == "sixty_percent_fixed_load"
    set_device_model!(template, DeviceModel(FlexibleLoad, FixDispatchablePowerLoad_60perc))
elseif load_model == "bid_in_load"
    set_device_model!(template, DeviceModel(FlexibleLoad, PSI.DispatchablePowerLoad))
else
    error("Unexpected value: $load_model")
end
...
# f = open("add_zero_demand_seg_Barton_dispatchable_day_2_update.txt","w"); print(f, sim.models.decision_models[1].internal.container.JuMPmodel); close(f)
...
results_output(res, sim, PTDF_matrix, emission_type, source_gen_data, result_file_path, start_date, "bid_in_load")