function results_output_curtail(
    ...
    load_model::String,
    data_center::Vector{PSY.PowerLoad})
    ...
    if load_output_model == "bid_in_load"
        flex_load = variable_values["ActivePowerVariable__CurtailableLoad"][!, names(variable_values["ActivePowerVariable__CurtailableLoad"]) .!= "DateTime"]
        CSV.write(joinpath(result_file_path, "curtailable_load_dispatch.csv"), flex_load)
    ...
    if load_output_model == "bid_in_load"
        ...
        for l in get_components(CurtailableLoad, sys)
        ...
        for l in get_components(CurtailableLoad, sys)
            name = get_name(l)
            baseline_load = get_component(PowerLoad, sys, name)
            baseline_load_ts = values(get_time_series_array(SingleTimeSeries, baseline_load, "max_active_power", start_time = start_date))[1:time_step] .* base_power
            load_payment_datacenter[!, Symbol(get_name(l))] = LMP[!, get_name(l)] .* (baseline_load_ts .- flex_load[!, get_name(l)])
        end
    ...
    if load_model == "bid_in_load"
        ...
        for l in get_components(CurtailableLoad, sys)
            for t in 1:time_step
                flex_load_ts = variable_values["ActivePowerVariable__CurtailableLoad"][!, names(variable_values["ActivePowerVariable__CurtailableLoad"]) .== get_name(l)][t, 1]
                ...
                if flex_load_ts <= 1/5 * max_load
                    value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + (332 * max_load - flex_load_ts * 10)
                elseif (flex_load_ts > 1/5 * max_load) & (flex_load_ts <= 2/5 * max_load)
                    value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + (332 * max_load - max_load/5 * 10 - (flex_load_ts-max_load/5) * 50)
                elseif (flex_load_ts > 2/5 * max_load) & (flex_load_ts <= 3/5 * max_load)
                    value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + (332 * max_load - max_load/5 * 10 + max_load/5 * 50 + (flex_load_ts-max_load/5*2) * 100)
                elseif (flex_load_ts > 3/5 * max_load) & (flex_load_ts <= 4/5 * max_load)
                    value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + (332 * max_load - max_load/5 * 10 + max_load/5 * 50 + max_load/5 * 100 + (flex_load_ts-max_load/5*3) * 500)
                elseif flex_load_ts > 4/5 * max_load
                    value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + (332 * max_load - max_load/5 * 10 + max_load/5 * 50 + max_load/5 * 100 + max_load/5 * 500 + (flex_load_ts-max_load/5*4) * 1000)
                end
            end
        end
        CSV.write(joinpath(result_file_path, "value_of_load_by_data_center.csv"), value_of_load)
    elseif load_model == "fixed_load"
        value_of_load = DataFrame()
        ...
        for l in data_center
            for t in 1:time_step
                max_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] .* base_power
                value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + 332 * max_load
            end
        end
        CSV.write(joinpath(result_file_path, "value_of_load_by_data_center.csv"), value_of_load)
    elseif load_model == "sixty_percent_fixed_load"
        value_of_load = DataFrame()
        for l in data_center
            value_of_load[!, Symbol(get_name(l))] = Float64[]
        end
        ...
        for l in data_center
            for t in 1:time_step
                max_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] .* base_power
                value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + 533.333333 * max_load
            end
        end
        CSV.write(joinpath(result_file_path, "value_of_load_by_data_center.csv"), value_of_load)
    end
    ...
    if load_output_model == "bid_in_load" || load_output_model == "constant_load"
        ...
        if load_model == "bid_in_load"
            for t in 1:time_step
                for l in get_components(CurtailableLoad, sys)
                    ...
                    max_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] .* base_power
                    if flex_load_ts <= 1/5 * max_load
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + (- flex_load_ts * 10)
                    elseif (flex_load_ts > 1/5 * max_load) & (flex_load_ts <= 2/5 * max_load)
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + (- max_load/5 * 10 - (flex_load_ts-max_load/5) * 50)
                    elseif (flex_load_ts > 2/5 * max_load) & (flex_load_ts <= 3/5 * max_load)
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + (- max_load/5 * 10 + max_load/5 * 50 + (flex_load_ts-max_load/5*2) * 100)
                    elseif (flex_load_ts > 3/5 * max_load) & (flex_load_ts <= 4/5 * max_load)
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + (- max_load/5 * 10 + max_load/5 * 50 + max_load/5 * 100 + (flex_load_ts-max_load/5*3) * 500)
                    elseif flex_load_ts > 4/5 * max_load
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + (- max_load/5 * 10 + max_load/5 * 50 + max_load/5 * 100 + max_load/5 * 500 + (flex_load_ts-max_load/5*4) * 1000)
                    end
                end
        ...
        elseif load_model == "sixty_percent_fixed_load"
            for t in 1:time_step
                for l in get_components(PowerLoad, sys, x -> PSY.get_available(x) == true)
                    if l in data_center
                        fixed_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] * base_power
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + 533.333333 * fixed_load
                    else
                        fixed_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] * base_power
                        value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + 332 * fixed_load
                    end
    ...
    if load_output_model == "bid_in_load"
        for t in 1:time_step
            ...
            for l in get_components(CurtailableLoad, sys)
                if get_bus(l).number < 200
                    total_load_by_region[t, Symbol("Region_1")] = total_load_by_region[t, Symbol("Region_1")] - variable_values["ActivePowerVariable__CurtailableLoad"][!, names(variable_values["ActivePowerVariable__CurtailableLoad"]) .== get_name(l)][t, 1]
                elseif get_bus(l).number >= 300
                    total_load_by_region[t, Symbol("Region_3")] = total_load_by_region[t, Symbol("Region_3")] + variable_values["ActivePowerVariable__CurtailableLoad"][!, names(variable_values["ActivePowerVariable__CurtailableLoad"]) .== get_name(l)][t, 1]
                else
                    total_load_by_region[t, Symbol("Region_2")] = total_load_by_region[t, Symbol("Region_2")] + variable_values["ActivePowerVariable__CurtailableLoad"][!, names(variable_values["ActivePowerVariable__CurtailableLoad"]) .== get_name(l)][t, 1]
                end
            end
        end
    elseif load_output_model == "constant_load"
