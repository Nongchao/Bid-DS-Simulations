function results_output(
    ...
    if load_output_model == "bid_in_load"
        flex_load = variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"][!, names(variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"]) .!= "DateTime"]
    ...
    if load_output_model == "bid_in_load"
        ...
        for l in get_components(ChlorAlkaliPlantWithBidLoad, sys)
            load_payment_datacenter[!, Symbol(get_name(get_bus(l)))] = zeros(time_step)
        end
        for l in get_components(PowerLoad, sys, x -> PSY.get_available(x) == true)
            ...
            total_load_payment = total_load_payment + sum(LMP[1:time_step, get_name(get_bus(l))] .* fixed_load)
        end
        for l in get_components(ChlorAlkaliPlantWithBidLoad, sys)
            total_load_payment = total_load_payment + sum(LMP[!, get_name(get_bus(l))] .* flex_load[!, get_name(l)])
        end
        for l in get_components(ChlorAlkaliPlantWithBidLoad, sys)
            load_payment_datacenter[!, Symbol(get_name(get_bus(l)))] = LMP[!, get_name(get_bus(l))] .* flex_load[!, get_name(l)]
        end
    ...
    if load_output_model == "bid_in_load"
        ...
        for l in get_components(FlexibleLoad, sys)
            for t in 1:time_step
                flex_load_ts = variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"][!, names(variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"]) .== get_name(l)][t, 1]
                ...
                # end
                value_of_load[1, Symbol(get_name(l))] = value_of_load[1, Symbol(get_name(l))] + flex_load_ts * chlor_alkali_bids
            end
        end
    ...
    if load_output_model == "bid_in_load"
        ...
        for t in 1:time_step
            for l in get_components(FlexibleLoad, sys)
                ...
                # end
                value_of_load_ts[t, :load_value] = value_of_load_ts[t, :load_value] + flex_load_ts * chlor_alkali_bids
            end
    ...
    for t in 1:time_step
        if load_output_model == "bid_in_load"
            ...
            for l in get_components(ChlorAlkaliPlantWithBidLoad, sys)
                load_payment_ts[t, :load_payment] = load_payment_ts[t, :load_payment] + LMP[t, get_name(get_bus(l))] * flex_load[t, get_name(l)]
            end
        elseif load_output_model == "constant_load"
            for l in get_components(PowerLoad, sys, x -> PSY.get_available(x) == true)
                fixed_load = values(get_time_series_array(SingleTimeSeries, l, "max_active_power", start_time = start_date))[t] * base_power
    ...
    if load_output_model == "bid_in_load"
        for t in 1:time_step
            ...
            for l in get_components(ChlorAlkaliPlantWithBidLoad, sys)
                if get_bus(l).number < 200
                    total_load_by_region[t, Symbol("Region_1")] = total_load_by_region[t, Symbol("Region_1")] + variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"][!, names(variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"]) .== get_name(l)][t, 1]
                elseif get_bus(l).number >= 300
                    total_load_by_region[t, Symbol("Region_3")] = total_load_by_region[t, Symbol("Region_3")] + variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"][!, names(variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"]) .== get_name(l)][t, 1]
                else
                    total_load_by_region[t, Symbol("Region_2")] = total_load_by_region[t, Symbol("Region_2")] + variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"][!, names(variable_values["ActivePowerVariable__ChlorAlkaliPlantWithBidLoad"]) .== get_name(l)][t, 1]
                end
            end
        end
    elseif load_output_model == "constant_load"
